...
def _verbmem(generator, model, tok, path, mask_id, device, cap):
    ...
    rls = [
        rouge_l_f1(
            generator.generate(model, tok, r["prompt"], mask_id, device,
                               max_new=128, chat=False),
            r["gt"])
        for r in rows
    ]
...
def _knowmem(generator, model, tok, path, qa_split, icl_split, mask_id, device, cap):
    ...
    shots = "".join(f"Question: {e['question']}\nAnswer: {e['answer']}\n\n" for e in icl)
    rows = list(load_dataset(path, "knowmem", split=qa_split))[:cap]
    rls = []
    for r in rows:
        pred = generator.generate(
            model, tok, f"{shots}Question: {r['question']}\nAnswer:",
            mask_id, device, max_new=32, chat=False)
        rls.append(rouge_l_f1(pred, r["answer"]))
    return sum(rls) / max(len(rls), 1)


@unlearning_metric(name="muse")
def muse(model, tokenizer, mask_id, device, generator=None,
         data_split="News", max_eval=100, **kw):
    ...
    return {
        ...
        "forget_knowmem_ROUGE": _knowmem(
            generator, model, tokenizer, path, "forget_qa", "forget_qa_icl",
            mask_id, device, max_eval),
        "retain_knowmem_ROUGE": _knowmem(
            generator, model, tokenizer, path, "retain_qa", "retain_qa_icl",
            mask_id, device, max_eval),
    }
